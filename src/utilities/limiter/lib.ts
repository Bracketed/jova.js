	...
	class PromisifiedStore implements Store {
		async increment(key: string): Promise<ClientRateLimitInfo> {
			return new Promise((resolve, reject) => {
				legacyStore.incr(key, (error: Error | undefined, totalHits: number, resetTime: Date | undefined) => {
...
const parseOptions = (passedOptions: Partial<Options>): Configuration => {
	...
	const config: Configuration = {
		...
		requestWasSuccessful: (_request: Request, response: Response): boolean => response.statusCode < 400,
		...
		async handler(request: Request, response: Response, _next: NextFunction, _optionsUsed: Options): Promise<void> {
			...
			const message: unknown =
				typeof config.message === 'function'
					? await (config.message as ValueDeterminingMiddleware<any>)(request, response)
					: config.message;
...
const rateLimit = (passedOptions?: Partial<Options>): RateLimitRequestHandler => {
	...
	(middleware as RateLimitRequestHandler).resetKey = config.store.resetKey.bind(config.store);
	(middleware as RateLimitRequestHandler).getKey =
		typeof config.store.get === 'function' ? config.store.get.bind(config.store) : getThrowFn;
